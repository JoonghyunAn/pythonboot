...
select datediff(curdate(), '2021-05-06');
...
select subdate(hire_date, interval 10 day) as 입사일마이너스십일
from emp
where dept_name = 'IT';
...
select emp_name,emp_id,hire_Date, date_format(hire_date, '%Y년 %m월 %d일') # format이 아니고 date_format! 접근방법은 맞았으나 함수명 실수
from emp
where emp_id = 200;
...
select emp_name, timestampdiff(month, hire_date, curdate()) as 근무개월
from emp;
