...
select distinct job  -- 중복된 것은 하나만 조회.
from   emp;

-- distinct: 행기준으로 중복을 제거해서 조회.
select distinct job,      -- 담당업무
	   dept_name 		  -- 부서명
from   emp;       

-- EMP 테이블에서 emp_id는 직원ID, emp_name은 직원이름, hire_date는 입사일,
--     salary는 급여, dept_name은 소속부서 별칭으로 조회결과를 출력 한다.
select  emp_id as "직원ID",  -- emp_id에서 조회한 것을 직원이름(별칭-alia)으로 사용자에게 전달.
        emp_name "직원이름",   -- as는 생략 가능
        hire_date "입사일",
        salary "급여",
        dept_name "소속 부서" 
        -- 별칭-식별자 규칙에 맞아야 한다. 규칙에 안맞는 이름의 별칭을 줄 경우 " "로 감싼다.
from    emp;
...
- 같은 컬럼을 여러번 조회할 수 있다.
************************************** */
select emp_name, 30+5 "값"
from   emp;

select 1 + 1, 30-20, 5*3, 10/4, 10%4, 10 mod 4, 10 div 3;
-- 산술 연산
select 1 + 1 + 10 + 20 + null;
-- 문자열 합치기 
select concat(100, '세', 300.2, '세');

-- EMP 테이블에서 직원의 이름(emp_name), 급여(salary) 그리고  
--      급여 + 1000 한 값을 조회.
select  emp_name, 
		concat('$',salary) as "salary", 
        concat('$', salary + 1000) as "salary+1000"
from    emp;
...
select salary, salary + 10000, salary * 10 from emp;
/* *************************************
where 절을 이용한 행 선택 
...
-- EMP 테이블에서 급여(salary)가 $10,000를 초과인 직원의 ID(emp_id), 
-- 이름(emp_name)과 급여(salary)를 조회
select  emp_id, emp_name, salary
from    emp
where   salary > 10000;
 
-- EMP 테이블에서 커미션비율(comm_pct)이 0.2~0.3 사이인
...
select emp_id, emp_name
from   emp
where  emp_name like '%!%%' escape '!'; -- !% -> %글자, !_ -> _
...
-- 급여(salary)가 $4,000에서 $8,000 사이에 포함된 
--  직원들의 ID(emp_id), 이름(emp_name)과 급여(salary)를 조회
select emp_id, emp_name, salary
from   emp
-- where  salary between 4000 and 8000;  -- 4000, 8000 포함
-- where salary not between 4000 and 8000;  -- 4000, 8000 포함안함
where salary not between 4000.1 and 7999.9;
...
where year(hire_date) = 2004; # hire_date에서 연도만 추출해서 비교
...
where not dept_name in ('Sales', 'IT', 'Shipping');

-- EMP 테이블에서 업무(job)가 'MAN'로 끝나는 직원의 ID(emp_id), 
--     이름(emp_name), 업무(job)를 조회
select emp_id, emp_name, job
from   emp
-- where  job like '%MAN'; -- MAN으로 끝나는 것.
-- where  job like 'MAN%'; -- MAN으로 시작하는 것.
-- where  job like '%MAN%'; -- MAN이 있는(포함된) 것.
where  job not like '%MAN'; -- like가 아닌 것. (MAN으로 끝나지 않는 것)
...
where comm_pct is not null; -- comp_pct가 null이 아닌 행. 
...
-- EMP 테이블에서 'SA_REP' 업무를 담당하는 직원들 중 급여(salary)가 $9,000인 
...
-- EMP 테이블에서 업무(job)가 'FI_ACCOUNT' 거나 급여(salary)가 $8,000 이상인 
...
-- EMP 테이블에서  'Sales' 부서 직원 중 
--     업무(job)가 'SA_MAN' 이고 급여가 $13,000 이하인 모든 정보를 조회
select * from emp
where dept_name = 'Sales'
and   job = 'SA_MAN'
and   salary <= 13000;
...
;
-- EMP 테이블에서, 'Executive'나 'Shipping'  
--   부서직원 중 급여(salary)가 6000 이상인 직원들의 모든 정보 조회. 
select * from emp
where dept_name in ('Executive', 'Shipping')
and   salary >= 6000;

-- EMP 테이블에서 업무(job)에 'MAN'이 들어가는 직원들 중에서 'Marketing' 이나'Sales'
--  부서에 소속된 직원들의 ID(emp_id), 이름(emp_name), 업무(job), 부서(dept_name)를 조회
select emp_id, 
	   emp_name,
       job,
       dept_name
...
-- EMP 테이블에서 업무(job)에 'MAN'이 들어가는 직원들 중에서 
-- salary가 10,000 이하 거나 2008년 이후 입사한 직원들에 정보를 조회.
select * 
from emp
...
-- (job에 MAN이 들어가고 salary가 10000이하) 거나 2008년 이후 입사한 직원
-- where  job like '%MAN%'
-- and    salary <= 10000
-- or     year(hire_date) >= 2008;
...
order by 3, 4 desc;  -- select 절의 column 순서로 지정.
-- order by job, salary desc, emp_id desc;
